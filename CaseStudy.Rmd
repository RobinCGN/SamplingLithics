...
legend("topright", c("Blade_prc", "Burned_prc", "Cortex_prc", "Mod_prc","+/- 5 %"),
       pch = c(0,1,2,3,15), col=c(rep("black",4),"lightgreen"),box.lty = 0)
```
As you can see the running estimates start of very variable and converge as more batches are beeing added. Linstädter et al. define a corridor of +/- 5 percent for numerical and +/-20 percent for percentages. If two times two consecutive values fall within this corridor the optimal sampling size has been reached. This limit as to be defined for every variable on its own.

## 7. Compare Estimates (samples) and overall values (original data se
